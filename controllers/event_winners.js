...
module.exports = function (is_mobile, user, page, docs, count, template) {
    template.render('event_winners', {
        ...
        pagination: function () {
            var temp = parseInt(count) / limit.winners;
            var totalPages = Math.floor(parseInt(count)/limit.winners);
            if (temp > totalPages) {
            ...
            for (var i = 0; i < pagination.length; i++) {
                if (pagination[i] !== "...") {
                    var url = "/event-winners?page=" + pagination[i];
                    array.push({
