...
module.exports = function (is_mobile, user, page, docs, count, template) {
    template.render('manage_notices', {
        layout: false,
        aws_s3_url: function () {
        ...
        docs: function () {
            ...
            for (var i = 0; i < docs.length; i++) {
                if (docs[i].finish_date < today_date) {
                    docs[i].is_finished = true;
                } else {
                    docs[i].is_finished = false;
                }
                docs[i].count_applied_resumes = docs[i].applied_resumes.length;
                docs[i].count_scrapped_users = docs[i].scrapped_users.length;
        ...
        pagination: function () {
            ...
            for (var i = 0; i < pagination.length; i++) {
                if (pagination[i] !== "...") {
                    var url = "/manage-notices?page=" + pagination[i];
                    array.push({
