...
module.exports = function (is_mobile, user, page, docs, count, template) {
    template.render('application_status', {
        layout: false,
        aws_s3_url: function () {
        ...
        docs: function () {
            for (var i = 0; i < docs.length; i++) {
                ...
                docs[i].is_opened = false;
                for (var j = 0; j < docs[i].applied_resumes.length; j++) {
                    if (docs[i].applied_resumes[j]._id === user.resume_id) {
                        docs[i].is_opened = docs[i].applied_resumes[j].is_opened;
                    }
                }
        ...
        pagination: function () {
            ...
            for (var i = 0; i < pagination.length; i++) {
                if (pagination[i] !== "...") {
                    var url = "/application-status?page=" + pagination[i];
                    array.push({
