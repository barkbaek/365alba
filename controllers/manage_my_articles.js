...
module.exports = function (is_mobile, user, page, docs, count, template) {
    template.render('manage_my_articles', {
        layout: false,
        aws_s3_url: function () {
        ...
        docs: function () {
            for (var i = 0; i < docs.length; i++) {
                ...
                if (docs[i].type === "albaboard") {
                    docs[i].type_korean = "알바톡톡";
                } else if (docs[i].type === "ceoboard") {
                    docs[i].type_korean = "사장님게시판";
                } else if (docs[i].type === "albareview") {
                    docs[i].type_korean = "알바후기";
                }
            }
            return docs;
        },
        ...
        pagination: function () {
            ...
            for (var i = 0; i < pagination.length; i++) {
                if (pagination[i] !== "...") {
                    var url = "/manage-my-articles?page=" + pagination[i];
                    array.push({
