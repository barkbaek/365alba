...
module.exports = function (is_mobile, user, doc, template) {
    template.render('single_ceoboard', {
        layout: false,
        aws_s3_url: function () {
        ...
        doc: function () {
            doc.is_updated_at = doc.created_at !== doc.updated_at;
            doc.count_comments = doc.comments.length;
            doc.id = doc._id.toString();
            doc.is_user = user === null ? false: user.is_user;
            doc.is_company = user === null ? false: (!user.is_user);
            doc.user_nick_name = user === null ? "" : user.nick_name;
            for (var i = 0; i < doc.comments.length; i++) {
                if (user === null) {
                    doc.comments[i].is_commenter = false;
                } else {
                    if (doc.comments[i].user_id === user._id.toString()) {
                        doc.comments[i].is_commenter = true;
                    } else {
                        doc.comments[i].is_commenter = false;
                    }
                }
                doc.comments[i].article_id = doc._id.toString();
            }
            return doc;
        },
        id: function () {
