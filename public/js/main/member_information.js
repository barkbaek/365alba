$(document).ready(function() {
    ...
    var s_cb = function (result) {
        if ( result['response'] === true ) {
            var doc = result.doc;
            if (doc.is_user === true) {
                $("#user-name").val(doc.user_name);
                $("#nick-name").val(doc.nick_name);
                if (doc.sex !== "") {
                    $("input:radio[name=sex]").filter("[value=" + doc.sex + "]").prop("checked", true);
                }
                $("#phone").val(doc.phone);
                if (doc.birth_year !== 0) {
                    $("#birth-year option[value=" + doc.birth_year + "]").attr("selected", "selected");
                    $("#birth-month option[value=" + doc.birth_month + "]").attr("selected", "selected");
                    $("#birth-day option[value=" + doc.birth_day + "]").attr("selected", "selected");
                }
            } else {
                $("#company-name").val(doc.company_name);
                $("#nick-name").val(doc.nick_name);
                $("#phone").val(doc.phone);
                $("#business-number").val(doc.business_number);
    ...
    var submit = function (e) {
        ...
        var s_cb = function (result) {
            if ( result['response'] === true ) {
                return show_bert("success", 2000, "성공적으로 회원정보를 변경하였습니다.");
            } else {
                if (result.msg === "server_error") {
                    return show_bert("danger", 2000, "서버 내부 오류가 발생하였습니다. 잠시 후 다시 시도해주세요.");
